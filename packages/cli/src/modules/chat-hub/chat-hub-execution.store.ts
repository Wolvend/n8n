...
export interface ChatHubExecutionContext {
	...
	/** Message ID this is a retry of (if applicable) */
	...
	isResuming: boolean;
	/** True if a new message needs to be created when execution resumes (external resume only) */
...
export class ChatHubExecutionStore {
	...
	async register(context: ChatHubExecutionContext): Promise<void> {
		...
		this.logger.debug(`Registered execution context for ${executionId}`);
	}
	...
	async update(
		...
		this.logger.debug(`Updated execution context for ${executionId}`);
	}
	...
	async remove(executionId: string): Promise<void> {
		...
		this.logger.debug(`Removed execution context for ${executionId}`);
	}
